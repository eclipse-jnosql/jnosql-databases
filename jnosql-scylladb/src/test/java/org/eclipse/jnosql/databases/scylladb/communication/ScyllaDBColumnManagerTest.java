...
public class ScyllaDBColumnManagerTest {

    public static final ConsistencyLevel CONSISTENCY_LEVEL = ConsistencyLevel.ONE;
    private ScyllaDBColumnManager entityManager;


    {
        ScyllaDBColumnManagerFactory managerFactory = ColumnDatabase.INSTANCE.get();
        entityManager = managerFactory.apply(Constants.KEY_SPACE);
    }

    @AfterEach
    void afterEach() {
        DefaultScyllaDBColumnManager manager = DefaultScyllaDBColumnManager.class.cast(entityManager);
    ...
    void shouldFindByIdWithConsistenceLevel() {
        ...
        assertThat(columns.stream().map(Element::value).map(Value::get).collect(toList()))
                .contains("ScyllaDB", 3.2, asList(1, 2, 3), 10L);
    ...
    void shouldPrepareStatement() {
        entityManager.insert(getColumnFamily());
        ScyllaDBPreparedStatement preparedStatement = entityManager.nativeQueryPrepare("select * from newKeySpace.newColumnFamily where id=?");
        ...
        assertThat(columns.stream().map(Element::value).map(Value::get)
                .collect(toList())).contains("ScyllaDB", 3.2, asList(1, 2, 3), 10L);
    }
    ...
    void shouldPagingState() {
        ...
        var query = select().from(Constants.COLUMN_FAMILY).build();
        ScyllaDBQuery scylladbQuery = ScyllaDBQuery.of(query);

        assertThat(scylladbQuery.getPagingState().isPresent()).isFalse();

        List<CommunicationEntity> entities = entityManager.select(scylladbQuery).toList();
        assertThat(entities.size()).isEqualTo(10);
        assertThat(scylladbQuery.getPagingState().isPresent()).isTrue();
    }
