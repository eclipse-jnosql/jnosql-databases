...
@ApplicationScoped
@Typed(ScyllaDBColumnEntityConverter.class)
class ScyllaDBColumnEntityConverter extends EntityConverter {
    ...
    private <T> void setUDTField(T instance, Optional<Element> column, FieldMetadata field) {
        if (column.isPresent() && org.eclipse.jnosql.databases.scylladb.communication.UDT.class.isInstance(column.get())) {
            org.eclipse.jnosql.databases.scylladb.communication.UDT udt =
                    org.eclipse.jnosql.databases.scylladb.communication.UDT.class.cast(column.get());
    ...
    protected AttributeFieldValue to(FieldMetadata field, Object entityInstance) {
        ...
        return annotation.<AttributeFieldValue>map(v -> new ScyllaDBUDTType(v, value, field))
