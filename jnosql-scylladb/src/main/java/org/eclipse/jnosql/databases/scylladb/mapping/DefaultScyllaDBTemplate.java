...
import org.eclipse.jnosql.databases.scylladb.communication.ScyllaDBPreparedStatement;
...
@Typed(ScyllaDBTemplate.class)
@ApplicationScoped
class DefaultScyllaDBTemplate extends AbstractSemiStructuredTemplate implements ScyllaDBTemplate {

    private final Instance<ScyllaDBColumnManager> manager;

    private final ScyllaDBColumnEntityConverter converter;
    ...
    @Inject
    DefaultScyllaDBTemplate(Instance<ScyllaDBColumnManager> manager,
                             ScyllaDBColumnEntityConverter converter,
    ...
    public <T> Stream<T> cql(String query, Object... params) {
        Objects.requireNonNull(query, "query is required");
        ScyllaDBPreparedStatement scylladbPrepareStatement = manager.get().nativeQueryPrepare(query);
        Stream<CommunicationEntity> entities = scylladbPrepareStatement.bind(params).executeQuery();
