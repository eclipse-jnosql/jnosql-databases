...
ArangoDB supports both Key-Value and Document APIs in this project. Its full installation, configuration, examples, and database-specific extensions are documented in the canonical xref:#arangodb-document[ArangoDB Document section].

[[couchbase-key-value]]
=== Couchbase

Couchbase supports both Key-Value and Document APIs in this project. Its full installation, configuration, examples, Template API, and repository support are documented in the canonical xref:#couchbase-document[Couchbase Document section].

[[dynamodb-key-value]]
=== DynamoDB

DynamoDB supports both Key-Value and Document APIs in this project. Its full installation, configuration, examples, capabilities, Template API, and repository support are documented in the canonical xref:#dynamodb-document[DynamoDB Document section].
...
Oracle NoSQL supports both Key-Value and Document APIs in this project. Its full installation, configuration, examples, Template API, and repository support are documented in the canonical xref:#oracle-nosql-document[Oracle NoSQL Document section].
...
- xref:#redis-single-node-configuration[`RedisConfigurations`] for single node configuration
+
[source,properties]
----
...
- xref:#redis-sentinel-configuration[`RedisSentinelConfigurations`] for sentinel configuration
+
[source,properties]
----
...
- xref:#redis-cluster-configuration[`RedisClusterConfigurations`] for cluster configuration
+
[source,properties]
----
...
- xref:#valkey-single-node-configuration[`ValkeyConfigurations`] for single node configuration
+
[source,properties]
----
# Single Node Configuration
...
- xref:#valkey-sentinel-configuration[`ValkeySentinelConfigurations`] for sentinel configuration
+
[source,properties]
----
# Sentinel Configuration
...
- xref:#valkey-cluster-configuration[`ValkeyClusterConfigurations`] for cluster configuration
+
[source,properties]
----
# Cluster Configuration
...
[[valkey-single-node-configuration]]
===== Single Node Configuration
...
https://www.scylladb.com/[ScyllaDB] is a distributed, highly available database compatible with Apache Cassandra's CQL protocol and designed for predictable, low-latency workloads.

This driver provides support for the *Column Family* NoSQL API using the ScyllaDB Java driver.
...
<dependency>
  <groupId>org.eclipse.jnosql.databases</groupId>
  <artifactId>jnosql-scylladb</artifactId>
...
This API provides the `ScyllaDBConfigurations` class to programmatically establish the credentials.
You can also establish properties using the https://microprofile.io/microprofile-config/[MicroProfile Config] specification.

[cols="ScyllaDB"]
|===
|Configuration property |Description

|`jnosql.scylladb.user`
|The user's user ID.

|`jnosql.scylladb.password`
|The user's password.

|`jnosql.scylladb.host`
|Database host. It is a prefix used to enumerate hosts. E.g.: jnosql.scylladb.host.1=localhost

|`jnosql.scylladb.name`
|The name of the application using the created session.

|`jnosql.scylladb.port`
|The ScyllaDB CQL port.

|`jnosql.scylladb.query`
|The CQL to execute when the configuration starts. It is a prefix. E.g.: jnosql.scylladb.query.1=<CQL>

|`jnosql.scylladb.data.center`
|The datacenter considered local by the load-balancing policy.
|===

This is an example using ScyllaDB with MicroProfile Config.

[source,properties]
----
jnosql.column.provider=org.eclipse.jnosql.databases.scylladb.communication.ScyllaDBConfiguration
jnosql.column.database=developers
jnosql.scylladb.host.1=localhost
jnosql.scylladb.port=9042
jnosql.scylladb.query.1=<CQL-QUERY>
jnosql.scylladb.query.2=<CQL-QUERY-2>
----

The configuration settings are the default behavior. To configure the manager programmatically, create an `@Alternative` class that implements `Supplier<ScyllaDBColumnManager>`.
...
@Alternative
@Priority(Interceptor.Priority.APPLICATION)
public class ManagerSupplier implements Supplier<ScyllaDBColumnManager> {

    @Produces
    public ScyllaDBColumnManager get() {
        Settings settings = Settings.builder()
                .put(ScyllaDBConfigurations.HOST.get() + ".1", "localhost")
                .put(ScyllaDBConfigurations.PORT.get(), 9042)
                .build();
...
The `@Column` annotation supports the `udt` attribute for mapping a field to a ScyllaDB user-defined type.
...
The `TimestampConverter` converts values to and from `java.util.Date`.
...
The `ScyllaDBTemplate` interface specializes `ColumnTemplate` and supports CQL operations.

[source,java]
----
@Inject
ScyllaDBTemplate template;
...
The `ScyllaDBRepository` interface extends `Repository` and supports CQL queries and consistency levels through the `@CQL` annotation.

[source,java]
----
@Repository
interface PersonRepository extends ScyllaDBRepository<Person, String> {
...
image::https://jnosql.github.io/img/logos/influxdb.png[InfluxDB,align="center" width=25%,height=25%"]
...
image::https://jnosql.github.io/img/logos/apache-io-tbd.png[Apache IoTDB,align="center" width=25%,height=25%"]
...
image::https://jnosql.github.io/img/logos/questdb.png[QuestDB,align="center" width=25%,height=25%"]
...
ArangoDB also supports graph access through the xref:#tinkerpop-integration[Apache TinkerPop integration]. Its canonical driver installation, configuration, and Document and Key-Value API documentation are in the xref:#arangodb-document[ArangoDB Document section].
...
- xref:#mongodb[MongoDB]


== Contributing a Database Driver

See xref:CONTRIBUTING_DATABASE.adoc[Contributing a New Eclipse JNoSQL Database Driver] for the driver requirements, package conventions, configuration classes, testing, manager suppliers, documentation, TCK guidance, and graph-provider notes.
