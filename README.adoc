...
The ```ScyllaDBRepository``` interface extends the ```Repository``` interface and supports CQL and consistency levels through the ```@CQL``` annotation.
...
The ```ScyllaDBTemplate``` interface is a specialization of the ```ColumnTemplate``` interface that allows using CQL.
