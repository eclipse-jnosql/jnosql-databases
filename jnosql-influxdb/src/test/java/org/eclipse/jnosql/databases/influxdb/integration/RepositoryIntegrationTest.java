...
class RepositoryIntegrationTest {
    ...
    void shouldInsertAndFindById() {
        Temperature temperature = new Temperature(Instant.now(), UUID.randomUUID().toString(), 21.5D);

        assertThat(repository.insert(temperature)).isEqualTo(temperature);
        assertThat(repository.findById(temperature.time())).contains(temperature);
    }

    @Test
    void shouldExecuteDerivedQuery() {
        String location = UUID.randomUUID().toString();
        List<Temperature> temperatures = List.of(
                new Temperature(Instant.now().minusSeconds(2), location, 21.5D),
                new Temperature(Instant.now().minusSeconds(1), location, 22.5D));
        repository.insertAll(temperatures);

        assertThat(repository.findByLocation(location)).containsExactlyInAnyOrderElementsOf(temperatures);
    }
