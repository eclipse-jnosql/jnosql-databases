...
- Add InfluxDB 3 driver for the JNoSQL Time Series Mapping API, with temporal identifiers, point conversion, batch writes, parameterized SQL queries, ordering, and limits
- Add Apache IoTDB driver for the JNoSQL Time Series Mapping API, using the native Table Model client, automatic table and column creation, temporal identifiers, tablet batch writes, filtering, ordering, limits, and temporal deletion
- Add QuestDB driver for the JNoSQL Time Series Mapping API, using native QWP ingestion and queries, automatic schema creation and evolution, temporal identifiers, native bind parameters, batch writes, filtering, ordering, limits, and updates
...
- Updated ArangoDB Java driver to 7.28.0
- Updated Couchbase Java client to 3.12.3
- Updated CouchDB HttpClient cache to 5.6.4
- Updated DynamoDB SDK to 2.55.1
- Updated Elasticsearch Java client to 9.5.4
- Updated Infinispan to 16.2.3
- Updated Jackson Annotations to 2.22
- Updated MongoDB Java driver to 5.12.0
- Updated Oracle NoSQL SDK to 5.4.24
- Updated OrientDB to 3.2.56
- Updated TinkerPop to 3.8.2
- Updated ANTLR runtime to 4.13.2
- Updated SLF4J test bindings to 2.0.19

=== Fixed

- Fixed potential query injection vulnerabilities and hardened parameter and identifier handling across generated database queries, including Gremlin, Oracle NoSQL, and Hazelcast predicates
- Fix Solr Document Manager operations to target the configured collection and avoid random ID collisions in integration tests
- Escape Solr query values and native query parameters to prevent Lucene query injection
- Bind TinkerPop Gremlin parameters through the script engine instead of interpolating values, preventing Gremlin query injection

== [1.1.18] - 2026-09-20

=== Added

- Add InfluxDB 3 support for the JNoSQL Time Series Mapping API, including temporal identifiers, point conversion, batch writes, parameterized SQL queries, ordering, and limits.
- Add Apache IoTDB support using the native Table Model client, with automatic table and column creation, temporal identifiers, tablet batch writes, filtering, ordering, limits, temporal deletion, and enum support.
- Add QuestDB support with native QWP ingestion and queries, automatic schema creation and evolution, temporal identifiers, native bind parameters, batch writes, filtering, ordering, limits, updates, and offset pagination.

=== Fixed

- Improve Gremlin parameter handling to prevent query injection issues.
- Fix query parameter handling to prevent query injection issues.
- Add enum support for Apache IoTDB.
- Add offset pagination support for QuestDB.

=== Changed

- Update Couchbase Java client to `3.12.3`.
- Update CouchDB HttpClient cache to `5.6.4`.
- Update DynamoDB SDK to `2.55.1`.
- Update Elasticsearch Java client to `9.5.4`.
- Update Infinispan to `16.2.3`.
- Update Jackson Annotations to `2.22`.
- Update MongoDB Java driver to `5.12.0`.
- Update Oracle NoSQL SDK to `5.4.24`.
- Update OrientDB to `3.2.56`.
- Update TinkerPop to `3.8.2`.
- Update ANTLR runtime to `4.13.2`.
- Update SLF4J test bindings to `2.0.19`.
...
* Upgraded Elasticsearch to `9.5.2`.
* Upgraded Jedis to `8.0.1` and migrated pooled and Sentinel clients to connection providers.
* Upgraded MongoDB to `5.11.0`.
* Upgraded HBase to `3.0.0`.
* Upgraded DynamoDB to `2.54.7`.
* Upgraded Neo4j to `6.2.1`.
* Updated database backports for the `1.1.x` release line.
* Updated license references across modules to Eclipse Public License 2.0.

=== Fixed

* Fixed Oracle NoSQL JSON `null` predicates to preserve the expected query semantics.
...
- Updated Couchbase to 3.12.2
- Updated CouchDB HttpClient cache to 5.6.3
