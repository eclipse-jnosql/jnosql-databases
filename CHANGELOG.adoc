...
- Upgrade Elasticsearch to `9.5.2`.
- Upgrade Jedis to `8.0.1`, including migration of pooled and Sentinel clients to connection providers.
- Upgrade MongoDB to `5.11.0`.
- Upgrade HBase to `3.0.0`.
- Upgrade DynamoDB to `2.54.7`.
- Upgrade Neo4j to `6.2.1`.
- Update database backports for the `1.1.x` release line.

=== Fixed

- Fix Oracle NoSQL JSON `null` predicates to preserve the expected query semantics.

=== Maintenance

- Update license references across modules to Eclipse Public License 2.0.
...
- Update Couchbabase version to 3.9.2
